...
class MFLars(MFConv):
    """Computes Lasso path using LARS algorithm for sparsity in female-
    specific coefficients.
    """

    def __init__(self, X_train, V_init, R=None, J=None, K=None, rank=None, max_iter=None,
                 lambda0=1.0, lambda1=None, lambda2=1.0, lambda3=0.0, name="MFLars", verbose=0):

        MFConv.__init__(self, name=name, X_train=X_train, V_init=V_init, R=R, J=J, K=K, 
                        rank=rank, lambda0=lambda0, lambda1=lambda1, lambda2=lambda2, lambda3=lambda3)
    ...
    def _update_U(self):
        ...
        reg = LassoLars(alpha=self.alpha, 
                        fit_path=False, 
                        normalize=True,
                        fit_intercept=False,
                        max_iter=self.max_iter).fit(self.V, self.S.T)
